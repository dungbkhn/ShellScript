...
if [ -f "$memtemp""/""$param1" ] ; then
	...
	for i in "${!names[@]}" ; do
		...
		for j in "${!names_remote[@]}" ; do
			...
			if [ "${isfile[$i]}" == "f" ] && [ "${names[$i]}" == "${names_remote[$j]}" ] ; then
					...
					if [ "${mtime[$i]}" == "${mtime_remote[$j]}" ] && [ "${filesize[$i]}" == "${filesize_remote[$j]}" ] && [ "${headmd5sum[$i]}" == "${headmd5sum_remote[$j]}" ] && [ "${tailmd5sum[$i]}" == "${tailmd5sum_remote[$j]}" ] ; then
						printf "./%s/1/%s/0/null/0\n" "${names[$i]}" "${names_remote[$j]}" >> "$memtemp""/""$param3"
					else
						printf "./%s/0/%s/%s/%s/%s\n" "${names[$i]}" "${names_remote[$j]}" "${filesize_remote[$j]}" "${tailmd5sum_remote[$j]}" "${mtime_remote[$j]}" >> "$memtemp""/""$param3"
					fi
					count=1
					found=1
					break
	...
	do
		if [ "${hassamefile[$i]}" -eq 0 ] && [ "${isfile[$i]}" == "f" ] ; then
			...
			headmd5sum_nt[$count]="${headmd5sum[$i]}"
			tailmd5sum_nt[$count]="${tailmd5sum[$i]}"
	...
	do
		if [ "${hassamefile_remote[$i]}" -eq 0 ] && [ "${isfile_remote[$i]}" == "f" ] ; then
			...
			headmd5sum_remote_nt[$count]="${headmd5sum_remote[$i]}"
			tailmd5sum_remote_nt[$count]="${tailmd5sum_remote[$i]}"
	...
	do
		...
		do
			if [ "${isselected_remote_nt[$j]}" -eq 0 ] && [ "${mtime_nt[$i]}" == "${mtime_remote_nt[$j]}" ] && [ "${filesize_nt[$i]}" == "${filesize_remote_nt[$j]}" ] && [ "${headmd5sum_nt[$i]}" == "${headmd5sum_remote_nt[$j]}" ] && [ "${tailmd5sum_nt[$i]}" == "${tailmd5sum_remote_nt[$j]}" ] ; then
				printf "./%s/2/%s/0/null/0\n" "${names_nt[$i]}" "${names_remote_nt[$j]}" >> "$memtemp""/""$param3"
